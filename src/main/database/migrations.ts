...
export const migrations: Migration[] = [
  ...
  {
    version: 7,
    name: 'invoice_customer_phone_snapshot',
    sql: `
      ALTER TABLE invoices
  ...
  },
  {
    ...
    sql: `
      CREATE TABLE invoice_returns (
        ...
        subtotal_ht_millimes INTEGER NOT NULL CHECK (subtotal_ht_millimes >= 0),
        tax_millimes INTEGER NOT NULL CHECK (tax_millimes >= 0),
        gross_ttc_millimes INTEGER NOT NULL CHECK (gross_ttc_millimes >= 0),
        global_discount_share_millimes INTEGER NOT NULL DEFAULT 0
          CHECK (global_discount_share_millimes >= 0),
        refund_ttc_millimes INTEGER NOT NULL CHECK (refund_ttc_millimes >= 0),
        created_at TEXT NOT NULL DEFAULT (datetime('now'))
      );

      CREATE TABLE invoice_return_lines (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        return_id INTEGER NOT NULL REFERENCES invoice_returns(id) ON DELETE RESTRICT,
        invoice_line_id INTEGER NOT NULL REFERENCES invoice_lines(id) ON DELETE RESTRICT,
        part_id INTEGER REFERENCES parts(id) ON DELETE SET NULL,
        reference_snapshot TEXT NOT NULL,
        designation_snapshot TEXT NOT NULL,
        quantity INTEGER NOT NULL CHECK (quantity > 0),
      ...
      CREATE INDEX idx_invoice_returns_invoice_created
        ON invoice_returns(invoice_id, created_at DESC);
      CREATE INDEX idx_invoice_return_lines_invoice_line
        ON invoice_return_lines(invoice_line_id);
    `
  }
]
